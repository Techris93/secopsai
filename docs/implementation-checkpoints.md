...
Initially added an authenticated stateless MCP server for a SecOpsAI ChatGPT app. It was subsequently generalized into the provider-neutral SecOpsAI MCP Gateway while preserving ChatGPT as a client profile. The gateway exposes nine read-only tools for findings, assets, changes, research cases, evidence matrices, and publication readiness; publishes OAuth protected-resource metadata; verifies JWT signature, issuer, audience, expiry, tenant, approved client, and per-tool scopes; and keeps the Core read token server-side. Production startup fails closed until an established OAuth provider and explicit client policy are configured.
