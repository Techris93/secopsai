...
The default autoresearch benchmark is the deterministic simulated attack dataset generated by `prepare.py`. Optional datasets such as BOTSv3 and OpenClaw are separate integration lanes and are not required for the main optimization loop.
...
If your goal is to improve the autoresearch loop itself, use only the simulated attack corpus produced by `prepare.py` and scored by `evaluate.py`.

That keeps optimization grounded on:

- deterministic labeled attacks
- reproducible benign background traffic
- fast regression runs
- no dependency on external Splunk exports or product-specific telemetry
...
Use it only as an optional realism check, not as the default autoresearch dataset.

If you added the BOTSv3 content CSVs (`ctf_questions.csv`, `ctf_answers.csv`, `ctf_hints.csv`), run:

```bash
python integrations/botsv3/botsv3_ingest.py
```
...
python integrations/botsv3/evaluate_botsv3.py

# 3) Run default benchmark as usual (synthetic baseline)
python evaluate.py
```

The `integrations/botsv3/evaluate_botsv3.py` helper temporarily swaps in BOTSv3 data for both
...
The OpenClaw path is additive: it does not replace the synthetic benchmark. The
new OpenClaw-specific rules in `detect.py` only act on
...
export_real_openclaw_native.py— Export local OpenClaw telemetry to native JSONL
...
run_openclaw_live.py          — One-step live OpenClaw pipeline runner
soc_store.py                  — Local SQLite SOC findings store + CLI helpers
integrations/botsv3/          — Optional BOTSv3 converters, evaluator, and CSVs
...
- BOTSv3 CSVs are isolated under `integrations/botsv3/` and tracked via Git LFS.
